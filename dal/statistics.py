...
def get_top_n_dict(statistics_dict, n):
    ...
    top_dict = dict(list(sorted_dict.items())[:n])  # Get the top n elements and their corresponding values
    return top_dict
...
def get_dataset_statistics(dataset_id):
    try:
        ...
        for feature_name in main_features_name_lst:
            ...
            if feature_name == 'Seed_match_canonical':
                feature_dist["statistics"]['canonical'] = feature_dist["statistics"]['True']
                feature_dist["statistics"]['noncanonical'] = feature_dist["statistics"]['False']
                feature_dist["statistics"].pop('True')
                feature_dist["statistics"].pop('False') 
